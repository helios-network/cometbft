...
func RPCRoutes(c *lrpc.Client) map[string]*rpcserver.RPCFunc {
	return map[string]*rpcserver.RPCFunc{
		...
		"blockchain_locate_not_empty_blocks": rpcserver.NewRPCFunc(makeBlockchainLocateNotEmptyBlocksInfoFunc(c), "minHeight,maxHeight", rpcserver.Cacheable()),
...
type rpcBlockchainLocateNotEmptyBlocksInfoFunc func(ctx *rpctypes.Context, minHeight, maxHeight int64) (*ctypes.ResultBlockchainInfo, error)

func makeBlockchainLocateNotEmptyBlocksInfoFunc(c *lrpc.Client) rpcBlockchainLocateNotEmptyBlocksInfoFunc {
	return func(ctx *rpctypes.Context, minHeight, maxHeight int64) (*ctypes.ResultBlockchainInfo, error) {
		return c.BlockchainLocateNotEmptyBlocksInfo(ctx.Context(), minHeight, maxHeight)
	}
}
