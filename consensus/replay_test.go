...
func testHandshakeReplay(t *testing.T, config *cfg.Config, nBlocks int, mode uint, testValidatorsChange bool) {
	...
	expectError := false
	if mode == 3 {
		pruned, _, _, err := store.PruneBlocks(2, state)
		require.NoError(t, err)
...
func (bs *mockBlockStore) PruneBlocks(height int64, _ sm.State) (uint64, [][]byte, int64, error) {
	pruned := uint64(0)
